...
for i in range(len(df)):
    ...
    add_fee_button = driver.find_element(By.XPATH, "// span[contains(text(), 'Add a new fee')]") # find Add fee button
    ...
    driver.implicitly_wait(10)
    fee_form = driver.find_elements(By.TAG_NAME, 'form')[0] # find all forms that a provider has, choose the newly created one to edit
    fee_type_block = fee_form.find_element(By.NAME, "fee_type").find_element(By.XPATH, '..') # find fee type block
    fee_type_block.click() #click fee type block
    fee_dropdown = driver.find_element(By.CSS_SELECTOR, "ul[role = 'listbox']") #find dropdown of fee types
    fee_types = fee_dropdown.find_elements(By.TAG_NAME, 'li') #find options of the dropdown
    #Iterate through the list of options to click the right one (based on input data)
    for f in range(len(fee_types)):
        fee_el = fee_dropdown.find_elements(By.TAG_NAME, 'li')[f] #find element
    ...
    field_grid = fee_form.find_elements(By.CSS_SELECTOR, "div[style *= 'grid-template-columns']")[1]  # locate grid with data fields on the fee
    fee_fields  = field_grid.find_elements(By.TAG_NAME, 'input')
    for ff in range(len(fee_fields)):
        ff_el = field_grid.find_elements(By.TAG_NAME, 'input')[ff].find_element(By.XPATH, '..').find_element(By.XPATH, '..')
        ff_label = ff_el.find_element(By.TAG_NAME, 'label').get_attribute('innerHTML') # get label element inner html to extract label name from there
        ...
        ff_input = ff_el.find_element(By.TAG_NAME, 'input') #find input form to put data there
